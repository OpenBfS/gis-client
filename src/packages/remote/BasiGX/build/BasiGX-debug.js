...
Ext.define('BasiGX.ux.ContextSensitiveHelp', {
    ...
    setContextHelp: function(additionalHelpKeys) {
        ...
        if (additionalHelpKeys) {
            Ext.Array.push(BasiGX.ux.ContextSensitiveHelp.existingHelpKeys, additionalHelpKeys);
        }
...
Ext.define("BasiGX.view.form.CoordinateTransform", {
    ...
    initComponent: function() {
        ...
        me.on('afterrender', function() {
            var transformvectorlayer = BasiGX.util.Layer.getLayerByName('transformvectorlayer');
    ...
    toggleBtnVisibility: function(field) {
        var allBtns = Ext.ComponentQuery.query('basigx-form-coordinatetransform button[name=transform]'),
    ...
    transform: function(evtOrBtnOrArray) {
        var me = Ext.ComponentQuery.query('basigx-form-coordinatetransform')[0],
            map = Ext.ComponentQuery.query('basigx-component-map')[0].getMap(),
            mapProjection = map.getView().getProjection(),
            fieldSets = me.query('fieldset'),
            transformvectorlayer = BasiGX.util.Layer.getLayerByName('transformvectorlayer'),
...
Ext.define("BasiGX.view.button.ToggleLegend", {
    ...
    */
    handler: function(button) {
        ...
        var legendPanel = button.up("basigx-panel-mapcontainer").down('basigx-panel-legendtree');
...
Ext.define("BasiGX.view.button.ZoomIn", {
    ...
    */
    handler: function(button) {
        var olMap = button.up("basigx-panel-mapcontainer").down('gx_map').getMap();
        var olView = button.up("basigx-panel-mapcontainer").down('gx_map').getView();
...
Ext.define("BasiGX.view.component.Map", {
    ...
    constructor: function(config) {
        ...
        if (!me.getMap()) {
            ...
            BasiGX.util.ConfigParser.autocreateLegends = config.autocreateLegends;
            BasiGX.util.ConfigParser.activeRouting = config.activeRouting;
...
Ext.define("BasiGX.view.container.NominatimSearch", {
    ...
    highlightFeature: function(tableView, record, item) {
        ...
        if (feature) {
            this.flashListenerKey = BasiGX.util.Animate.flashFeature(feature, 1000, radius);
        }
    },
    /**
     *
...
Ext.define("BasiGX.view.form.Print", {
    ...
    layerFilter: function(layer) {
        ...
        var inTree = nonOpaque && (layer.get(BasiGX.util.Layer.KEY_DISPLAY_IN_LAYERSWITCHER) !== false);
...
Ext.define("BasiGX.view.panel.LayerSetChooser", {
    ...
    initComponent: function() {
        ...
        this.callParent(arguments);
        // add listeners
        this.down('basigx-view-layerset').on('select', this.onLayerSetSelect);
        this.down('basigx-view-layerset').on('itemclick', this.onLayerSetClick);
        this.down('basigx-view-layerset').on('itemdblclick', this.onLayerSetDblClick);
    },
    /**
    ...
    onLayerSetSelect: function(view, rec, index, opts) {
        this.up('basigx-panel-layersetchooser').fireEvent('select', view, rec, index, opts);
    },
    /**
     * Just firing an event on the panel.
    ...
    onLayerSetClick: function(view, rec, item, index, evt, opts) {
        view.up('basigx-panel-layersetchooser').fireEvent('itemclick', view, rec, item, index, evt, opts);
    },
    /**
     * Just firing an event on the panel.
    ...
    onLayerSetDblClick: function(view, rec, item, index, evt, opts) {
        view.up('basigx-panel-layersetchooser').fireEvent('itemdblclick', view, rec, item, index, evt, opts);
    },
    /**
     *
     */
...
Ext.define("BasiGX.view.panel.MapContainer", {
    ...
    toggleOverviewMap: function(button, pressed) {
        var ovm = button.up("basigx-panel-mapcontainer").down('gx_overviewmap');
        if (pressed) {
            ovm.show(button);
        } else {
            ovm.hide(button);
