...
var TableCreator = /** @class */ (function () {
    ...
    TableCreator.prototype.createRequired = function () {
        var dao = new spatialReferenceSystemDao_1.SpatialReferenceSystemDao(this.geopackage);
     ...
     * @return {boolean}
     */
    TableCreator.prototype.createTileScaling = function () {
    ...
    TableCreator.prototype.createTable = function (creationScriptName) {
        var success = true;
        var scripts = TableCreator.tableCreationScripts[creationScriptName];
    ...
    TableCreator.prototype.createUserTable = function (userTable) {
        ...
        var result = connection.tableExists(userTable.getTableName());
        if (result) {
            throw new Error('Table already exists and cannot be created: ' + userTable.getTableName());
        }
        // Build the create table sql
        var sql = coreSQLUtils_1.CoreSQLUtils.createTableSQL(userTable);
        // Create the table
     ...
     */
    TableCreator.prototype.dropTable = function (table) {
        coreSQLUtils_1.CoreSQLUtils.dropTable(this.connection, table);
    };
